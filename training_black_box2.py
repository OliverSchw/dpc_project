...
if __name__ == "__main__":
    ...
    for i in range(len(key_idx)):
        ...
        node = NeuralEulerODE([4, 32, 32, 32, 2], key=key)
        fin_node, losses, val_losses = train_model(node, featurize_node, validation)
        eqx.tree_serialise_leaves(
            f"final_models/black_box/Model_3_32_{key_idx[i]}.eqx",
            fin_node,
        )
        jnp.save(
            f"final_models/black_box/losses/Model_3_32_{key_idx[i]}.npy",
            losses,
        )
        jnp.save(
            f"final_models/black_box/val_losses/Model_3_32_{key_idx[i]}.npy",
            val_losses,
        )
