...
def train_model(node):
    ...
    fin_node, fin_opt_state, fin_keys, losses, val_losses = mtrainer.fit(node, opt_state, keys, validate_every=10_000)
...
if __name__ == "__main__":
    ...
    for i in range(len(key_idx)):
        ...
        node = ExpertModel(motor_env=motor_env, psi_layer_sizes=[2, 32, 32, 32, 2], key=key)
        fin_node, losses = train_model(node)
        eqx.tree_serialise_leaves(
            f"final_models/grey_box/Model_3_32_{key_idx[i]}.eqx",
            fin_node,
        )
        jnp.save(
            f"final_models/grey_box/losses/Model_3_32_{key_idx[i]}.npy",
            losses,
        )
