...
def train_model(key):
    ...
    node = ExpertModel(motor_env=motor_env, psi_layer_sizes=[2, 32, 32, 2], key=key)
...
if __name__ == "__main__":
    ...
    for i in range(len(key_idx)):
        ...
        eqx.tree_serialise_leaves(
            f"final_models/grey_box/Model_2_32_{key_idx[i]}.eqx",
            fin_node,
        )
        jnp.save(
            f"final_models/grey_box/losses/Model_2_32_{key_idx[i]}.npy",
            losses,
        )
