...
def grad_loss(model, true_obs, actions, tau, featurize):
    ...
    pred_obs = jax.vmap(rollout_traj_node, in_axes=(None, 0, 0, None))(model, true_obs[:, 0, :], actions, tau)
    ...
    # probably only necessary for angular properties like theta in pendulum

    return jnp.mean((pred_obs - true_obs) ** 2)
